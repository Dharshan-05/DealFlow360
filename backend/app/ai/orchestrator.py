...
from app.ai.security import sanitize_untrusted_input, check_permission_for_tool, PromptInjectionError
...
class AIOrchestrator:
    ...
    def process_query(self, request: AIQueryRequest) -> AIQueryResponse:
        # Phase 314: Prompt Injection Defense
        try:
        ...
        # Phase 298: Copilot Context Building
        conversation = self._get_or_create_conversation(request.conversation_id)
        
        system_prompt = "You are DealFlow360 AI Copilot. You explain data from tools and provide verified business insights. Never hallucinate facts."
        ...
        # Handle Tool Call (Phase 311, 312, 313)
        if response.tool_calls:
            ...
            # Check if it needs confirmation (Phase 313 Guarded Action)
            ...
            try:
                ...
                # Second pass to get explanation
                ...
                self._log_audit("TOOL_EXECUTION", tool_name=name, action_payload=args, success=False, error_message=str(e))
                return AIQueryResponse(answer=f"Error executing tool: {str(e)}")
